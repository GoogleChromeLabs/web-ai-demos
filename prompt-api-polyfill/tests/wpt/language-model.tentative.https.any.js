...
promise_test(async (t) => {
  assert_true(
    ...
    'LanguageModel should be available in global scope'
  );
  ...
  assert_true(
    ...
    'maxTemperature should be greater than 0'
  );
  assert_true(
    params.defaultTemperature >= 0,
    'defaultTemperature should be greater than or equal to 0'
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    ['available', 'downloadable', 'downloading'].includes(availability),
    'Availability should be a known status'
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    response.includes('4'),
    "Response should contain common answer '4'"
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    response.toLowerCase().includes('blue'),
    'Session should preserve history'
  );
...
promise_test(async (t) => {
  ...
  const usage = await session.measureInputUsage(
    'Test string for token counting'
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    ...
    'Cloned session should preserve history'
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    aiResponse.toLowerCase().includes('text'),
    'OCR should identify text in image'
  );
...
promise_test(async (t) => {
  ...
  assert_true(
    aiResponse.toLowerCase().includes('country'),
    'Audio transcription should be accurate'
  );
