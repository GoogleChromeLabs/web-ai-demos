...
describe.each(activeBackends)(
  'Prompt API Polyfill Browser ($name Backend)',
  (backend) => {
    beforeAll(async () => {
    ...
    describe('Static Methods', () => {
      ...
      it('LanguageModel.create() should create a session', async () => {
        const { defaultTemperature, defaultTopK, maxTemperature, maxTopK } =
    ...
    describe('Instance Methods', () => {
      ...
      it('session should maintain conversation history', async () => {
        const historySession = await window.LanguageModel.create({
          temperature: 0,
          topK: 1,
        });
        try {
          ...
          const finalResponse = await historySession.prompt(
            'What were the two cities I just asked about?'
          );
          ...
          const aiResponse = await multimodalSession.prompt([
            {
              ...
              content: [
                {
                  type: 'text',
                  value: 'What does the text in this image say?',
                },
      ...
      it('session.clone() should create a new session and preserve history', async () => {
        ...
        try {
          // Establish history
          await originalSession.prompt(
            'My secret name is "Prompty McPromptface".'
          );
    ...
    describe('Structured Output', () => {
      it('session.prompt() should respect responseConstraint (JSON Schema)', async () => {
        ...
        const response = await session.prompt(
          'What is the capital of Germany? Respond in JSON.',
          {
